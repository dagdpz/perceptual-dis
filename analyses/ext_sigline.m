function ext_sigline(xs,lbl,h,yv, coordinates, StarColor)
...
hold on
if strcmp(coordinates, 'x')
xs=[xs(1);xs(2)];
plot(xs,[1;1]*y*1.1,'-k', 'LineWidth',3);%line
else
xs=[xs(1);xs(2)];
plot([1;1]*y*1.1,xs,'-k', 'LineWidth',3);%line
end
%plot(mean(xs), y*1.15, '*k')% the sig star sign
if lbl
    if strcmp(coordinates, 'x')
    text(mean(xs), y*1.12, lbl,'FontSize',30, 'FontWeight','bold','Color' ,StarColor)% the sig star sign
    else
    text(y*1.14, mean(xs), lbl,'FontSize',30, 'FontWeight','bold','Color' ,StarColor)% the sig star sign
    end
end
...
%     else
% plot([y,y*1.07],[1;1]*xs(1),'-k', 'LineWidth',2);%left edge drop
% plot([y,y*1.07],[1;1]*xs(2),'-k', 'LineWidth',2);%right edge drop        
%     end
hold off
%--------------------------------------------------------------------------
